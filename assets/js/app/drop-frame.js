...
$(document).ready(function(){
    dndCanvas.setHeight(2 * window.innerHeight);
    dndCanvas.setWidth(2 * window.innerWidth);
    ...
    $(document).on({
        ...
        keyup: function(e){
            if (e.originalEvent.keyCode == 46){
                var object = dndCanvas.getActiveObject();
                if (object != null){
                    if (object.class == "element"){
                        dndCanvas.remove(object.tickButton, object.newPoint, object);
                        dndCanvas.renderAll();
                    }
                }
            }
...
dndCanvas.on("mouse:up", function(e){
    ...
    if (targetObj != null && e.e.pageX < 0){
        dndCanvas.remove(targetObj.newPoint, targetObj.tickButton, targetObj);
        dndCanvas.renderAll();
        targetObj = null;
    }
    saveFrame();
